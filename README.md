...
### ⚡ 창을 먼저 띄우고, 도로망은 켤 때만 로딩

앱은 **켜자마자 즉시 뜹니다.** 페이지를 그릴 때는 무거운 도로망(OSMnx)을 전혀 건드리지 않고,
기본은 **직선거리(km)** 로 안내하기 때문입니다. (세종시처럼 좁은 지역은 근접 순위가 거의 동일)

**도로 소요시간(차량 N분)이 필요할 때만**, 사이드바의 **"🚗 도로 소요시간 사용"** 스위치를 켜세요.
켠 뒤 **첫 검색에서** 도로망을 불러오며 최초 1~3분 걸립니다(이후 캐시되어 빠름). 실패하면 직선거리로 자동 전환됩니다.

> ⏱️ **도로망 로딩을 수 초로 단축** — 로컬에서 `python build_graph.py` 를 1회 실행해 만든
> `sejong_drive.graphml` 을 커밋해두면, 켰을 때 재다운로드 없이 파일에서 바로 로딩합니다.
> 무료 호스팅(1GB)에서 도로망이 불안정하면 이 방식이나 직선거리(기본) 유지를 권장합니다.
