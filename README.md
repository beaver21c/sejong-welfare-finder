...
- **가까운 기관 안내** — 직선거리(km) 기준으로 즉시 안내 *(도로망 소요시간은 선택 기능)*
...
기본 배포에는 무거운 도로망 패키지(OSMnx)가 빠져 있어 **앱이 몇 초 만에 뜨고 안정적**입니다.
거리는 **직선거리(km)** 로 안내하며, 세종시처럼 좁은 지역은 근접 순위가 거의 동일해 실용상 충분합니다.

> **도로 소요시간(차량 N분)이 꼭 필요하면** `requirements.txt`의 `osmnx`/`networkx` 주석을 해제하고
> `packages.txt`에 `libspatialindex-dev` 를 추가하세요. 단 **무료 호스팅에서는** 딸려오는 지리 패키지
> 때문에 빌드가 느리거나 메모리 부족으로 앱이 **'계속 로딩'** 상태가 될 수 있습니다. 이때는
> `python build_graph.py` 로 `sejong_drive.graphml` 을 미리 만들어 커밋하면 재다운로드를 피할 수 있고,
> 그래도 불안정하면 유료 리소스 상향 또는 경량(직선거리) 유지가 안전합니다.
